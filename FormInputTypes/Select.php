 ...
 */    
    public function getJsValidation($idPrefix)
    {
        $code = '';
        if ($this->configuration['multiply'] == true) {
            return 'var j = 0;if (form["'.$this->prefix.$this->name.'"].length != undefined) {for (var i in form["'.$this->prefix.$this->name.'"]) {if ((form["'.$this->prefix.$this->name.'"][i].selected) || (form["'.$this->prefix.$this->name.'"][i].checked)) {j++;}}}if (j == 0) {errors["'.$this->prefix.$this->name.'"] = '.json_encode($this->configuration['requiredError']).';}'.self::JS_EOL;
        }
        return '';
    }
